...
Several platforms currently address fake news and misinformation detection to varying degrees. The table below presents a comparative analysis:

| System                      | Description                                                                                                          | Type                   | Limitations                                          |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------- | ---------------------- | ---------------------------------------------------- |
| **Snopes**                  | One of the oldest and most respected fact-checking websites, covering viral claims, urban legends, and news stories. | Web-based, manual      | No mobile app; slow human-driven verification        |
| **FactCheck.org**           | Nonpartisan fact-checking site focusing on political claims and statements by public figures in the USA.             | Web-based, manual      | US-centric; no automated AI analysis                 |
| **NewsGuard**               | Browser extension that rates news websites for credibility using human analyst teams.                                | Browser extension      | Requires desktop browser; no NLP; subscription cost  |
| **Google Fact Check Tools** | Aggregates structured fact checks from verified publishers and surfaces them in Google Search results.               | Search-integrated      | Passive tool; no interactive features; English-only  |
| **PolitiFact**              | Focuses on US political claims using a "Truth-O-Meter" scale rated by journalists.                                   | Web-based, manual      | Limited scope; no mobile app; US-only                |
| **ClaimBuster**             | AI-assisted tool for identifying check-worthy factual claims in text content.                                        | Web-based, AI-assisted | Narrow scope (detection only); no mobile; no scoring |
...
| **RAM**              | Minimum 8 GB (16 GB recommended for emulator + server) |
...
| **PHP**              | ≥ 8.2                                                              |
| **MySQL**            | 8.0+                                                               |
| **Python Runtime**   | 3.11+ (for FastAPI service)                                        |
| **SSL Certificate**  | Required for production HTTPS (Let's Encrypt)                      |
...
| Requirement               | Detail                                                                                           |
| ------------------------- | ------------------------------------------------------------------------------------------------ |
| **API Protocol**          | RESTful HTTP/HTTPS API (JSON payloads)                                                           |
...
- **Engagement Through Gamification:** The integration of the Fact vs Fiction quiz, badge systems, and a points leaderboard encourages habitual engagement and long-term user retention, addressing user drop-off challenges common in information-utility apps.
...
| **Backend Hosting**        | £5–£20/month                    | VPS (e.g., DigitalOcean, Railway, Fly.io)    |
| **Database (MySQL)**       | £0–£10/month                    | Included with most hosting plans             |
| **FastAPI Hosting**        | £0–£10/month                    | Lightweight Python server                    |
| **Firebase**               | £0–£25/month                    | Generous free tier; pay-as-you-go for scale  |
| **AI / NLP API**           | £0–£50/month                    | Free tiers available (OpenAI, Google Gemini) |
| **Domain Name**            | ~£10/year                       | One-time/annual cost                         |
| **SSL Certificate**        | £0                              | Let's Encrypt (free)                         |
| **App Store Distribution** | £25 (Google) / £99/year (Apple) | One-time / annual developer account          |
...
| Decision                    | Rationale                                                                                                                        |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| **Flutter Frontend**        | Single codebase for Android + iOS reduces development time and maintenance overhead                                              |
| **Laravel REST API**        | Mature PHP framework with built-in ORM (Eloquent), queue management, and JWT integration                                         |
| **FastAPI AI Microservice** | Python's ML ecosystem (PyTorch, HuggingFace) far exceeds PHP; isolating AI in a dedicated service prevents blocking the main API |
...
| ---------------- | --------------------------------------------------- | ---------------------------------------------- |
| `User`           | id, name, email, password_hash, locale, points      | register(), login(), logout(), updateProfile() |
| `Article`        | id, url, title, content, credibility_score, source  | analyse(), bookmark(), share()                 |
| `AnalysisResult` | id, article_id, user_id, score, summary, created_at | getScore(), getSummary()                       |
| `Bookmark`       | id, user_id, article_id, created_at                 | save(), remove()                               |
| `ChatSession`    | id, user_id, messages[], created_at                 | sendMessage(), getHistory()                    |
| `QuizGame`       | id, user_id, score, questions[], completed_at       | start(), submitAnswer(), getScore()            |
| `Badge`          | id, name, description, icon_url, criteria           | award(), check()                               |
...
| --------------------------------- | ------- | ------------------------------------------------ |
| **Flutter**                       | SDK 3.x | Cross-platform mobile framework (Android + iOS)  |
...
| **Dio**                           | 5.x     | HTTP client with interceptors for JWT injection  |
| **flutter_secure_storage**        | 9.x     | Secure local storage for JWT tokens              |
| **google_sign_in**                | 6.x     | Google OAuth on Android and iOS                  |
...
| **Eloquent ORM**      | (Laravel built-in) | Database object-relational mapping                   |
| **Laravel Queue**     | (Laravel built-in) | Async job processing for analysis tasks              |
...
| **Android Studio** | Hedgehog+ | Flutter development IDE, Android AVD   |
...
The following table summarises the development progress as of the interim submission date:

| Feature / Module                    | Status         | Notes                                         |
| ----------------------------------- | -------------- | --------------------------------------------- |
| Laravel Project Initialisation      | ✅ Complete    | Laravel 12 project set up with Composer       |
| MySQL Database Schema               | ✅ Complete    | Migrations for all core entities created      |
| JWT Authentication (Login/Register) | ✅ Complete    | Endpoints tested via Postman                  |
| Google OAuth (Socialite)            | ✅ Complete    | Redirect and callback flows implemented       |
| User Profile API                    | ✅ Complete    | GET/PUT /api/user endpoints functional        |
| Article Analysis API                | ✅ Complete    | URL and text submission endpoints complete    |
| FastAPI AI Microservice             | ✅ Complete    | BERT model inference service running          |
| AI Chat API (Proxy)                 | ✅ Complete    | Laravel proxies to OpenAI/Gemini API          |
| Bookmark API                        | ✅ Complete    | CRUD endpoints for bookmarks                  |
| News Digest API                     | 🔄 In Progress | News aggregation from external RSS/APIs       |
| Search API                          | 🔄 In Progress | Full-text search implementation ongoing       |
| Flutter Project Setup               | ✅ Complete    | Feature-based folder structure established    |
| Flutter Auth Screens                | ✅ Complete    | Login, Register, Google Sign-In screens       |
| Flutter News Digest Screen          | ✅ Complete    | Displays news from backend                    |
| Flutter Article Analysis Screen     | ✅ Complete    | URL/text input + score display                |
| Flutter AI Chat Screen              | ✅ Complete    | Conversational UI with message bubbles        |
| Flutter Bookmark Screen             | ✅ Complete    | Saved articles list with removal              |
| Flutter Game/Quiz Screen            | 🔄 In Progress | Quiz logic 80% complete; scoring remaining    |
| Flutter Profile Screen              | ✅ Complete    | User details and settings view                |
| Multilingual Support (EN/SI/TA)     | 🔄 In Progress | English complete; Sinhala/Tamil pending       |
| Firebase Firestore Integration      | 🔄 In Progress | Collection structure designed; read/write WIP |
| Dark Mode Support                   | ✅ Complete    | Theme toggle implemented via Riverpod         |
...
The following Gantt timeline outlines the planned and actual project phases:

| Phase                             | Activities                                                     | Planned Period | Status            |
| --------------------------------- | -------------------------------------------------------------- | -------------- | ----------------- |
| **Phase 1: Initiation**           | Requirements gathering, PID submission, project planning       | Sep–Oct 2025   | ✅ Complete       |
| **Phase 2: Design**               | System architecture, wireframes, database schema, UML diagrams | Oct–Nov 2025   | ✅ Complete       |
| **Phase 3: Backend Development**  | Laravel API, MySQL schema, JWT auth, Google OAuth              | Nov–Dec 2025   | ✅ Complete       |
| **Phase 4: AI Service**           | FastAPI setup, BERT model integration, inference API           | Dec 2025       | ✅ Complete       |
| **Phase 5: Frontend Development** | Flutter screens, state management, API integration             | Jan–Feb 2026   | ✅ Complete (85%) |
| **Phase 6: Feature Completion**   | Search, multilingual, quiz, Firebase integration               | Feb–Mar 2026   | 🔄 In Progress    |
| **Phase 7: Testing**              | Unit testing, integration testing, user acceptance testing     | Mar–Apr 2026   | ⏳ Upcoming       |
| **Phase 8: Final Submission**     | Report writing, presentation, deployment                       | Apr 2026       | ⏳ Upcoming       |
...
| -------------------------------- | ----------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| **AI Model Performance**         | Initial BERT model inference was slow (>5s per request) on a standard VPS                             | Implemented async queued jobs in Laravel; optimised model to use `DistilBERT` for faster inference |
| **Cross-Platform Auth**          | Google Sign-In required different configuration for Android and iOS (different OAuth client IDs)      | Configured separate OAuth clients per platform in Firebase and Google Cloud Console                |
| **Race Conditions in State**     | Riverpod providers occasionally produced stale state in multi-screen navigation flows                 | Refactored to use `AsyncNotifier` pattern with proper invalidation on navigation events            |
| **Firebase + Laravel Dual Auth** | Managing authentication tokens for both Firebase SDK (real-time features) and Laravel JWT was complex | Introduced a token exchange endpoint where Laravel backend exchanges a Firebase token for a JWT    |
| **CORS Configuration**           | Early testing revealed CORS policy issues between Flutter web build and Laravel API                   | Configured strict CORS middleware with proper origin allowlist in Laravel                          |
...
| Backend API           | Laravel 12 (PHP 8.2+)               | 12.x    |
| AI Microservice       | FastAPI + BERT (HuggingFace)        | 0.110+  |
| Authentication        | JWT (tymon/jwt-auth) + Google OAuth | 2.x     |
| Database (Relational) | MySQL                               | 8.0+    |
| Database (Real-time)  | Firebase Firestore                  | Latest  |
| State Management      | Flutter Riverpod                    | 2.x     |
| Version Control       | Git / GitHub                        | —       |
